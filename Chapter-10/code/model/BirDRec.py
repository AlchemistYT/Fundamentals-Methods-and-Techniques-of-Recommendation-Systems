...
class BirDRec(nn.Module):
    def __init__(self, config, forward_encoder: ContextEncoder, backward_encoder: ContextEncoder = None):
        ...
        self.train_neg_num = self.config['train_neg_num']
        self.target_neg_num = self.config['target_neg_num']
    ...
    def forward(self, train_batch, epoch_num):
        user_id, hist_item_ids, pos_target, replace_candidates, sample_indices = train_batch
        sample_size = self.config['train_neg_num']
        cand_size = replace_candidates.shape[1]
        sampled_neg_targets = torch.randint(low=1, high=self.config['item_num'], size=(user_id.shape[0], sample_size)).type(torch.long)
        sample_indices = sample_indices.unsqueeze(1)
        ...
        if epoch_num >= self.corr_epoch:
            if self.rectify_input:
                ...
                if self.rectify_target:  # rectify input and rectify target
                    if self.self_ensemble: # rectify input and rectify target, with ensemble
                        loss, rectified_target_ratio, update_pool_indices, target_keep_condition = self.correct_target_with_ensemble(
                            rectified_pos_score, rectified_neg_score, rectified_cand_score, rectified_ema_pos_score, rectified_ema_neg_scores)

                    reliable_confidence, unreliable_confidence = self.get_confidence(self.forward_encoder, user_id, hist_item_ids, pos_target)
                ...
                temp_tensor = (torch.zeros(input_select_condition.shape).to(self.device) - 1)

                instances_unreliable_inputs = torch.where(input_select_condition > 0, temp_tensor, sample_indices)
                instances_unreliable_targets = torch.where(target_keep_condition > 0, temp_tensor, sample_indices)

                return loss, sample_indices, rectified_input_ratio, rectified_target_ratio, update_pool_indices, instances_unreliable_inputs, instances_unreliable_targets, reliable_confidence, unreliable_confidence, target_keep_condition
                    ...
                    if self.self_ensemble: # do not rectify input, rectify target, with ensemble
                        ema_pos_score_last, _, _, _, ema_neg_scores_last, _ = \
                            self.get_various_scores_last_and_second_last(self.moving_avg_model.model, user_id, hist_item_ids,
                                                                         pos_target,
                                                                         replace_candidates, sampled_neg_targets)
                        loss, rectified_target_ratio, update_pool_indices, target_keep_condition = \
                ...
                return loss, sample_indices, rectified_input_ratio, rectified_target_ratio, update_pool_indices, None, None
            ...
            loss = -(torch.log(torch.sigmoid(pos_score - neg_scores[:, cand_size:cand_size + 2])))
            loss = loss.sum(dim=1, keepdim=True)
            return loss, sample_indices, rectified_input_ratio, rectified_target_ratio, None, None, None

    def get_confidence(self, model, user_id, hist_item_ids, pos_target):
        # [bs, 32]
        _, item_embeds = model.obtain_embeds()
        # [bs, dim]
        context_embed = model(user_id, hist_item_ids, rectify=False)
        # [bs, hidden_size]
        pos_target_embed = item_embeds[pos_target]
        # [bs, 1]
        pos_score = torch.mul(context_embed, pos_target_embed).sum(dim=1, keepdim=True)

        sampled_neg_targets = torch.randint(low=1, high=self.config['item_num'],
                                            size=(user_id.shape[0], 32)).type(torch.long)
        # [bs, 32, dim]
        neg_item_embeds = item_embeds[sampled_neg_targets]
        # [bs, 32]
        neg_scores = torch.mul(context_embed.unsqueeze(1), neg_item_embeds).sum(dim=2, keepdim=False)

        # [bs, 33]
        cat_scores = torch.concat([pos_score, neg_scores], dim=1)
        cat_scores_softmax = torch.softmax(cat_scores, dim=1)
        pos_score_softmax = cat_scores_softmax[:, 0]

        reliable_confidence = pos_score_softmax
        unreliable_confidence = 1 - pos_score_softmax

        return reliable_confidence, unreliable_confidence
    ...
    def correct_target_with_ensemble(self, pos_score, neg_scores, cand_scores, ema_pos_score, ema_neg_scores):
        ...
        target_neg_scores = sorted_neg_scores[:, 0: int(self.target_neg_num)]
        target_keep_condition = (ema_pos_score > delete_judger_score).long().to(self.device)
        rectified_target_ratio = 1 - (target_keep_condition.sum() / target_keep_condition.shape[0])
        raw_loss = self.bpr_loss(pos_score, target_neg_scores, weighted=False)
        corrected_loss = torch.mul(target_keep_condition, raw_loss)

        return corrected_loss, rectified_target_ratio, update_pool_indices, target_keep_condition
    ...
    def data_analysis(self, train_batch):
        user_id, hist_item_ids, pos_target, replace_candidates, sample_idices = train_batch
        sample_size = self.config['analysis_neg_num']
        sampled_neg_targets = torch.randint(low=1, high=self.config['item_num'], size=(user_id.shape[0], sample_size)).type(torch.long)
        # [bs, 1], [bs, neg_num]
        pos_score, _, neg_scores = \
            self.get_various_scores_last(self.forward_encoder, user_id, hist_item_ids, pos_target,
                                         replace_candidates, sampled_neg_targets)
        return pos_score.detach(), neg_scores.detach()
